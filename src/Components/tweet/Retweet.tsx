...
import RetweetIcon from "../mics/RetweetIcon";
...
export default function Retweet({ tweetId, tweetAuthor }: TweetOptionsProps) {
  ...
  const { isFetched, data, isLoading} = useQuery({
    queryKey: queryKey,
  ...
  const mutation = useMutation({
    mutationFn: (variables: any) =>
      updateRetweets(
        tweetId,
        tweetAuthor,
        variables.tokenOwnerId,
        variables.isRetweeted
      ),
    onMutate: () => {
  ...
  const handleRetweet = () => {
    ...
    const isRetweetedBy = retweetedBy?.some(
  ...
  return (
    <>
      ...
      >
        ...
        <motion.span
          animate={{ scale: isRetweeted ? [0, 1.2, 1] : 0 }}
          transition={{ duration: 0.25 }}
        />
