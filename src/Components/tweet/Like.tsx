...
export default function Like({ tweetId, tweetAuthor }: TweetOptionsProps) {
    ...
    return (
        <>
            <motion.button
                className={`border-none bg-inherit flex justify-center text-twitterMuted opacity-75 items-center cursor-pointer like ${isLiked ? "font-[900] text-twitterLike" : ""}`}
                onClick={handleLike}
                whileTap={{ scale: 0.9 }}
            ...
            >
                {isLiked ? (
                    <motion.span className="flex justify-center items-center" animate={{ scale: [1, 1.5, 1.2, 1] }} transition={{ duration: 0.25 }}>
                        <FaHeart  className='p-2 hover:text-twitterLike hover:bg-twitterLikeBackground' />
                    </motion.span>
                ) : (
